...
const getCurFiles = (
  dir: string,
  SuffixIncludes: string[] = [],
  unFileIncludes: string[] = []
) => {
  if (!dir) return [];
 ...
 * @return {*}
 */
const isNotReadme = (filename: string) =>
...
const createREADME = (dir: string) => {
  ...
  // 生成文件内容
  const content = Template.READMETemplate(
    configs,
...
const hasIndexMd = (path: string) => {
  ...
  return getCurFiles(path, ["md"])
    .map((item) => getFileNameByPath(item))
    .includes("index");
};

/**
 * @description: 将windows路径中的“\\”替换成linux系统中的“/”
 * @return {bool} 是windows返回true，其他返回linux
 */
const fitPlatform = (path: string) => {
